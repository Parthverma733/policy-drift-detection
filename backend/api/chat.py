...
@router.post("/start")
async def start_chat_session(policy_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error starting chat session: {str(e)}"
        )


@router.post("/message")
async def send_message(session_id: str, message: str, db=Depends(get_db)):
...
async def get_chat_messages(session_id: str, db=Depends(get_db)):
    """Get all messages for a chat session."""
    try:
        messages_collection = db["chat_messages"]
        cursor = messages_collection.find({"session_id": ObjectId(session_id)}).sort(
            "timestamp", 1
        )
